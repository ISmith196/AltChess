...
Public Class Form1
    ...
    Private m_boolGameRunning As Boolean = True 'will be set to true when game is running. Will be set false when game ends.
    ...
    Private Sub EndGame(winner As Boolean)
        ...
        MsgBox("" & strPlayer & " won!", MsgBoxStyle.Exclamation, "Game Over!")
        ActiveControl = pbxLastSpace
        m_p1Pieces.Clear()
        m_p2Pieces.Clear()
    ...
    Function GetNewTime(time As String, playerTurn As Boolean) As String
        Dim arrTime() As String = time.Split(":")
        Dim intNewTime As Integer = (CInt(arrTime(0)) * 60) + CInt(arrTime(1) - 1)
        If intNewTime = 0 Then
            turnTimer.Stop()
            EndGame(Not playerTurn)
        End If
    ...
    Private Sub turnTimer_Tick(sender As Object, e As EventArgs) Handles turnTimer.Tick
        If P1Turn = True Then
            lblP1Time.Text = GetNewTime(lblP1Time.Text, P1Turn)
        Else
            lblP2Time.Text = GetNewTime(lblP2Time.Text, P1Turn)
        End If
    End Sub
    ...
    Private Sub Piece_Click(sender As Object, e As EventArgs) Handles pbxP1piece1.Click, pbxP2piece1.Click
        ...
        If ActiveControl.Name.Equals(activePiece.Name) = False Then 'either option 1 or 2
            ...
            If activePiece.Name(4).Equals(ActiveControl.Name(4)) Then
                If m_boolHasMoved = True Then
                    MsgBox("You cannot select this piece.", MsgBoxStyle.Information, "Ally piece selected")
                Else
                    DrawBoard()
                    ActiveControl = activePiece
